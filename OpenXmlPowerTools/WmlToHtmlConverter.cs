...
namespace OpenXmlPowerTools
{
    ...
    public class WmlToHtmlConverterSettings
    {
        ...
        public bool SetParagraphId = true;
        public bool SetEmptytParagraphId = false;   // TODO
    ...
    public static class WmlToHtmlConverter
    {
        ...
        /// <summary>
        /// Return a list since a a
        /// </summary>
        /// <param name="wordDoc"></param>
        /// <param name="settings"></param>
        /// <param name="node"></param>
        /// <param name="suppressTrailingWhiteSpace"></param>
        /// <param name="currentMarginLeft"></param>
        /// <returns></returns>
        ...
        private static object ProcessBreak(XElement element)
        {
            ...
            // flag the break if it was a page break so we can handle it later.
            ...
            XElement paragraph, XName elementName, bool suppressTrailingWhiteSpace, decimal currentMarginLeft, bool isBidi)
        {
            ...
            if (settings.SetParagraphId)
            {
                if (settings.SetEmptytParagraphId == false)
                {
                    // TODO: Check if paragraph has no text and skip setting id
                }
                var paraId = paragraph.Attributes(W14.paraId).FirstOrDefault();
                if (paraId != null)
                {
                    paraElement.Add(new XAttribute("id", paraId.Value));
                }
            }
        ...
        /// <summary>
        /// Inserts a Section if none was found. TODO: Verify functionality
        /// </summary>
        private static SectionAnnotation InitializeSectionAnnotation(IEnumerable<XElement> reverseDescendants, string paperSize = "A4")
        {
        ...
        {
            var grouped = new LinkedList<IGrouping<string, XElement>>(
                elements
                .GroupAdjacent(e =>
                {
            ...
            var node = grouped.First;
            while (node != null)
            {
                if (node.Value.Key == "")
                {
                    newContent.Add(
                        node.Value.Select(e => ConvertToHtmlTransform(wordDoc, settings, e, false, currentMarginLeft))
                        );
                }

                var last = node.Value.Count() - 1;
                newContent.Add(
                    node.Value.Select((e, i) => ConvertToHtmlTransform(wordDoc, settings, e, i != last, currentMarginLeft))
                    );
